<template>
    ...
    <v-card style="width:450px;" outlined>
        ...
        <v-card-title v-if="value._links">
            정산 # {{value._links.self.href.split("/")[value._links.self.href.split("/").length - 1]}}
        </v-card-title >
        <v-card-title v-else>
            정산
        </v-card-title >
        ...
        <v-card-text>
            ...
            <시간별계량치Manager offline label="시간별계량치" v-model="value.시간별계량치" :editMode="editMode" @change="change"/>
        </v-card-text>
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/정산'), this.value)
                        } else {
